...
function chatbot_html_box($autoload = false){
    ...
    if($button_color){
        echo "<style>:root { --chatbot-button-color: ".esc_html($button_color)."!important;}</style>";
    }
    if($button_background_color){
        echo "<style>:root { --chatbot-button-bg-color: ".esc_html($button_background_color)."!important;}</style>";
    }
    if($header_color){
        echo "<style>:root { --chatbot-header-color: ".esc_html($header_color)."!important;}</style>";
    }
    if($header_background_color){
        echo "<style>:root { --chatbot-header-bg-color: ".esc_html($header_background_color)."!important;}</style>";
    }
    ...
    ?>
    <div class="chatbot-opener mode-<?php echo esc_html($default_mode); ?> <?php echo ($opened_by_default === 'status-closed') ? '' : 'status-closed'; ?> <?php echo esc_html($css_class); ?>" id="chatbot-bubble">
        <div class="chatbot-bubble-inner">
            <div class="chatbot-icon"><?php echo esc_html($icon); ?></div>
            <div class="chatbot-name"><?php echo esc_html($bot_name); ?></div>
        </div>
    </div>

    <div id="chatbot-container" class="position-<?php echo esc_html($position); ?> mode-<?php echo esc_html($default_mode); ?> <?php echo esc_html($opened_by_default); ?> <?php echo esc_html($css_class); ?>">
        ...
        <div id="chatbot-messages">
            ...
            <?php 
            if(empty($error)){
                echo '<div class="chatbot-icon">'.(!empty($options['icon']) ? '<div class="custom-icon"><img src="'.esc_html($options['icon']).'"></div>' : '<i class="fi fi-rr-chatbot-speech-bubble"></i>').'</div>';
        ...
        <?php if(empty($error)){ ?>
            ...
            <input type="text" id="chatbot-input" placeholder="<?php echo esc_html($message_placeholder); ?>" />
			<input type="hidden" id="chatbot-nonce" value="<?php echo esc_html(wp_create_nonce('handle_chatbot_conversation_nonce')); ?>">
            <button id="chatbot-send-btn" disabled><?php echo esc_html($send_button); ?></button>
        </div>
        <?php } ?>
    </div>
    <?php
